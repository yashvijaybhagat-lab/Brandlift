...
function StoreCard({ name, tag, color, delay }: { name: string; tag: string; color: string; delay: number }) {
  return (
    ...
    >
      <div className="w-8 h-8 rounded-lg flex-shrink-0" style={{ background: color + '22', border: `1px solid ${color}44` }}>
        <div className="w-full h-full rounded-lg" style={{ background: color + '33' }} />
      </div>
      <div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center py-20">
        ...
        <div>
          ...
          >
            ...
            <a
              href="#how-it-works"
              className="text-[14px] text-[#71717A] hover:text-[#FAFAFA] transition-colors duration-150"
            >
              See how it works →
            </a>
          </motion.div>
